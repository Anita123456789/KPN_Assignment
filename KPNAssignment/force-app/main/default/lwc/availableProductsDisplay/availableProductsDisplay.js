			...
			if (this.PRICEBOOK_ID ){
                ...
                .then (result => {					
                    for (let itdtCol of result) {
						...
						this.items.push({ Name: itdtCol.Product2.Name,
                                          UnitPrice: itdtCol.UnitPrice,
                                          id: itdtCol.Product2Id
                                        });
					}					
    ...
    addProducts () {
		try{		
			...
			if (selected.length ==0) {				
				this.displayMassageEvent('Error',this.labels.productNotSelected,'error');					 
			} else {
				...
				if (this.orderItemProducts) {
					...
					.then (result => {						
						if (result) {							
							...
							this.displayMassageEvent('Successful',this.labels.successfulProdAdded,'success');							
						}else {							
							this.displayMassageEvent('Error',this.labels.error_Message,'error');
						}
					})
